 ...
 * </ul> */
public final class RFC3161Connection {
	...
	private final static Logger LOGGER = Logger.getLogger(RFC3161Connection.class.getName());

    public final TimeStampRequestGenerator tsqGenerator;
    ...
    public RFC3161Connection(final boolean requireCert,
                     final String policy,
                     final URI tsa,
                     final String tsaUsr,
                     final String tsaPwd,
                     final RFC3161RequestExtension[] extensions,
    ...
    public RFC3161Connection(final boolean requireCert,
        ...
        if (extensions != null) {
        	for (final RFC3161RequestExtension ext : extensions) {
    ...
    public RFC3161Connection(final RFC3161Params params) {
    	this(
    ...
    */
    ...
    private static byte[] getTSAResponseHttp(final byte[] requestBytes, final URLConnection conn) throws IOException {
         ...
         final byte[] respBytes = FileUtils.toByteArray(conn.getInputStream());
	...
	private void configureHttpsConnection(final URLConnection conn) throws IOException {
    	...
    	try {
    		...
    		setSSLSocketFactoryMethod.invoke(conn, new MySocketFactory(sc.getSocketFactory()));
  ...
  // XMAS. Use old Protocol and Ciphers
  public static class MySocketFactory extends javax.net.ssl.SSLSocketFactory {

    public final javax.net.ssl.SSLSocketFactory parent;

    /**
       * 
       */
    public MySocketFactory(javax.net.ssl.SSLSocketFactory parent) {
      super();
      this.parent = parent;
    }

    private Socket initializeSocket(Socket ssl) {
    ...
    public Socket createSocket(Socket s, String host, int port, boolean autoClose)
      ...
      return initializeSocket(parent.createSocket(s, host, port, autoClose));
    ...
    public Socket createSocket(String host, int port) throws IOException, UnknownHostException {
      ...
      return initializeSocket(parent.createSocket(host, port));
    }
    ...
    public Socket createSocket(String host, int port, InetAddress localHost, int localPort)
      ...
      return initializeSocket(parent.createSocket(host, port, localHost, localPort));
    ...
    public Socket createSocket(InetAddress address, int port, InetAddress localAddress,
      ...
      return initializeSocket(parent.createSocket(address, port, localAddress, localPort));
    }
      ...
      System.out.println(" XYZ ZZZ hashAlgorithm => " + hashAlgorithm);
    	...
    	final TimeStampRequest request = this.tsqGenerator.generate(
         ...
         try {
            ...
            throw new Exception("Error obteniendo la respuesta de la TSA: " + e, e); //$NON-NLS-1$
         }
         ...
         try {
            ...
            throw new Exception("Error validando la respuesta de la TSA: " + e, e); //$NON-NLS-1$
         }
         final PKIFailureInfo failure = response.getFailInfo();
         final int value = failure == null ? 0 : failure.intValue();
         if (value != 0) {
             throw new Exception("Respuesta invalida de la TSA ('" + this.tsaURL + "') con el codigo " + value); //$NON-NLS-1$ //$NON-NLS-2$
         }
         ...
         if (tsToken == null) {
             throw new Exception("La respuesta de la TSA ('" + this.tsaURL + "') no es un sello de tiempo valido: " + new String(rawResponse)); //$NON-NLS-1$ //$NON-NLS-2$
         }
