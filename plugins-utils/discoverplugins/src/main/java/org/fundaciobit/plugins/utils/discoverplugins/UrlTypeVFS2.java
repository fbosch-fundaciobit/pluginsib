 ...
 * 
 */
public class UrlTypeVFS2 implements UrlType {
  public final static String[] REPLACE_EXTENSION = new String[] { ".ear/", ".jar/", ".war/",
      ".sar/", ".har/", ".par/" };
  ...
  public Vfs.Dir createDir(final URL url) {
    try {
      ...
      Vfs.Dir dir = createDir(new java.io.File(adaptedUrl.getFile()));
      return dir;
    } catch (Exception e) {
      e.printStackTrace();
      try {
  ...
  Vfs.Dir createDir(java.io.File file) {
    try {
      return file.exists() && file.canRead() ? file.isDirectory() ? new SystemDir(file)
